...
vector<int> get_random_duplicate_values(int total, pair<int,int> val_range, pair<int,int> duplication_range){
    int remaining = total;
    vector<int> nums;
    ...
    while(remaining){
        ...
        // only duplicate the element with 40% probability
        if(rand()%10 >= 6)
        duplicationCount = get_random_value(duplication_range);

        // to keep the exact total length 
        duplicationCount = min(remaining, duplicationCount);

        // update remaining numbers variable
        remaining -= duplicationCount;

        int value = get_random_value(val_range);
        // add the value duplicationCount times
        while(duplicationCount--)
...
vector<vector<int>> get_random_duplicate_set(int total, pair<int,int> n_range, pair<int,int> val_range, pair<int,int> duplication_range){
    ...
    for(int i=0; i<total; i++){
        int n = get_random_value(n_range);
        vector<int> nums = get_random_duplicate_values(n, val_range, duplication_range);
        numsVec.push_back(nums);
    }
